...
- **后端**: FastAPI 0.115.6，入口 `backend/main.py`，路由注册 `backend/api/v1/__init__.py`
- **前端**: Vue 3.5 + TypeScript 6.0 + Vite 8.0，入口 `frontend/src/main.ts`，根组件 `frontend/src/App.vue`
- **API 响应格式**: `{ "code": 0, "message": "success", "data": any }`，错误时 `code` 为非零值
- **路由前缀**: `/api/v1`
- **权限代码格式**: JWT Bearer Token（管理端 `Authorization: Bearer <token>`）+ API Key（Chat 应用端 `X-API-Key` 或 `Authorization: Bearer <apikey>`）
- **API 路由模块**: auth, configs, llm-configs, mcp-servers, skills, knowledge-bases, agents, workflows, apps, chat, traces, metrics, ws
- **数据模型**: 15 张表 — Agent, User, Workflow, App, Session, WorkflowTrace, AuditLog, SysConfig, LlmConfig, McpServerRegistry, AgentMcpLink, KnowledgeBase, AgentKbLink, ContentBlock, SkillRegistry, AgentSkillLink
- **ORM**: Tortoise ORM + MySQL 8.0 (utf8mb4)，配置在 `backend/config.py` 的 `TORTOISE_ORM` 中，新增模型模块必须在此注册
- **前端路由模块**: dashboard, agents, workflows, apps, resources(mcp-servers/knowledge-bases/skills), monitor(traces/chat-test), settings(llm-configs)
- **设计文档**: `system_design_docs/` 下有 20 份设计文档，`system_design_docs/README.md` 是索引
