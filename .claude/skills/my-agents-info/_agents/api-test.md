...
- **项目**: {项目名称}
- **后端入口**: {后端入口文件}，启动命令 {启动命令}
- **API 基础路径**: {API 基础路径}
- **数据库**: {数据库类型及名称}，ORM: {ORM 名称}
- **认证**: {认证方式}，登录接口 {登录接口路径}
- **测试账号**: {测试账号信息}
...
- ⚠致命 测试前必须先登录获取 token，登录接口 {登录接口路径}，请求体 {登录请求体示例}
- ⚠致命 如果 `settings.LOCAL_DEBUG = True`，认证中间件跳过，可不传 token；但部署环境必须传 Bearer Token
- ⚠致命 测试数据清理顺序必须与创建顺序相反（先删子数据后删父数据），否则关联约束导致删除失败
- ⚠隐蔽 `wrap_response` 装饰器缺少 `functools.wraps` 会导致 422 错误，测试时注意区分业务错误和框架错误
- ⚠隐蔽 Service 层调用 async 方法时忘记 `await` 会导致返回协程对象而非实际数据
- ⚠隐蔽 校验逻辑与自动填充的执行顺序错误会导致必填字段校验失效
