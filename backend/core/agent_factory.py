    ...
    async def create_local_agent(self, agent_config: Dict[str, Any], event_queue: asyncio.Queue = None):
        ...
        if available_workers:
            ...
            routing_instruction = (
                ...
                f"可用的子代理：\n{worker_list}\n\n"
                f"调度规则：\n"
                f"1. 如果用户只是打招呼（如「你好」「hello」「hi」「在吗」等），"
                f"或者不需要任何子代理能力的简单闲聊，你可以直接回复并输出 NEXT_AGENT: end，无需路由到子代理。\n"
                f"2. 如果用户问题确实需要子代理的专业能力，选择一个最合适的子代理，输出 NEXT_AGENT: <名称>\n"
                f"3. 当子代理返回结果后，检查结果是否已完整回答用户问题。"
